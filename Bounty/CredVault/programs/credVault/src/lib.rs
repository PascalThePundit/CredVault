...
pub mod credVault {
    ...
    pub fn mint_proof_of_work(
        ...
        .invoke(&CreateMasterEditionV3InstructionArgs {
            max_supply: Some(0), // Unlimited supply means it's a one-of-a-kind NFT
        })?;
...
pub struct MintCredential<'info> {
    ...
    #[account(mut, seeds = [b"issuer", issuer.issuer_pubkey.as_ref()], bump = issuer.bump, has_one = issuer_pubkey)]
    pub issuer: Account<'info, IssuerAccount>,
    
    #[account(mut)]
    pub issuer_account: Signer<'info>, // The actual issuer account signing the transaction
    ...
    #[account(
        ...
        bump,
        mint::decimals = 0,
        mint::freeze_authority = issuer_account,
        mint::mint_authority = issuer_account,
    )]
    ...
    #[account(
        ...
        token::authority = issuer_account,
    )]
    pub credential_token_account: Account<'info, TokenAccount>,
    
    #[account(
        mut,
        seeds = [b"master-edition", mint.key().as_ref()],
        bump,
        seeds::program = mpl_token_metadata::ID,
    )]
